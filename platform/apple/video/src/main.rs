//! Yama Video Service - Apple Silicon
//!
...
pub use decoder::{create_decoder, VideoToolboxDecoder};
...
impl VideoPipeline {
    /// Create a new video pipeline with platform decoder.
    fn new(name: &str, config: &VideoSourceConfig) -> Result<Self> {
        ...
        info!("Decoder configured: {:?}", decoder.capabilities());

        // Build GStreamer pipeline for capture (decoder is handled by platform trait)
...
impl VideoService {
    ...
    async fn new(config: &Config) -> Result<Self> {
        ...
        let event_bus = Arc::new(
            EventBusClient::connect(&config.event_bus_socket, "video-apple")
                .await
        ...
        let health = HealthReporter::new(event_bus.clone(), "video-apple");
    ...
    async fn run(mut self) -> Result<()> {
        ...
        // Set up frame callbacks for each pipeline
        for (name, pipeline) in &self.pipelines {
                            ...
                            Ok(sample) => {
                                if let Some(buffer) = sample.buffer() {
                                    ...
                                    let meta = VideoFrameMeta {
                                        ...
                                        frame_number: 0, // Would track this properly
                                        pts: None,
...
async fn main() -> Result<()> {
    ...
    info!("Starting Yama Video Service (Apple Silicon)");
    info!(
