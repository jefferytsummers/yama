...
//!
//! This service provides LLM inference using TensorRT-LLM or llama.cpp
//! with CUDA acceleration on NVIDIA Jetson.
...
pub use engine::{create_inference_engine, CudaInferenceEngine};
...
struct Config {
    ...
    /// Inference server URL (TensorRT-LLM server or llama.cpp server).
    server_url: Option<String>,
    /// Model path (if using local inference).
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            model_path: None,
            default_model: "llama-3.2-7b".to_string(),
            max_concurrent: 4,
        }
    }
}
...
impl InferenceService {
    async fn new(config: &Config) -> Result<Self> {
        ...
        let event_bus = Arc::new(
            EventBusClient::connect(&config.event_bus_socket, "inference-jetson")
                .await
        ...
        let health = HealthReporter::new(event_bus.clone(), "inference-jetson");

        // Initialize platform-specific CUDA inference engine
        ...
        let engine = create_inference_engine(server_url, &config.default_model)?;

        info!(
            "CUDA inference engine initialized: {} on {}",
    ...
    async fn run(mut self) -> Result<()> {
        info!("Starting inference service (Jetson CUDA)");
        ...
        loop {
            tokio::select! {
                _ = stats_interval.tick() => {
                    ...
                    info!(
                        "GPU Memory usage: {}/{} MB ({} models loaded)",
...
async fn main() -> Result<()> {
    ...
    info!("Starting Yama Inference Service (NVIDIA Jetson)");
